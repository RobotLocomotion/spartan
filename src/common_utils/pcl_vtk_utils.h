...
static typename pcl::PointCloud<PointType>::Ptr PointCloudFromPolyData(
...
//----------------------------------------------------------------------------
template <typename PointType>
static typename pcl::PointCloud<PointType>::Ptr PointCloudFromPolyDataWithNormals(
    vtkPolyData *polyData) {
  const vtkIdType numberOfPoints = polyData->GetNumberOfPoints();
...
static Eigen::Matrix<double, 6, -1> LoadMatrixWithVTKWithNormals(
  ...
  if (cloudPolyData->GetPointData()->GetNormals()) {
    sceneCloud = PointCloudFromPolyDataWithNormals<pcl::PointNormal>(cloudPolyData);
  } else {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr sceneCloudWithoutNormals =
        PointCloudFromPolyData<pcl::PointXYZ>(cloudPolyData);
