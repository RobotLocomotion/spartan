...
def viz_predicted_depth1(path,model_path,sleep =.1,filter_files= None,img_height=480,img_width=640,save_dir = None): #add file filter for specific logs
	model = network.load_trained_model(weights_path = model_path)
	...
	im3 = ax3.imshow(blank)
	im4 = ax4.imshow(blank)
	...
	for i in range(len(samples)):
		...
		network.apply_mask(predicted_prob_map,gtdepth,threshold)
		im1.set_data(depth/3500)
		im2.set_data(gtdepth*3500)
		im3.set_data(gt_copy*3500)
		#im4.set_data(rgb)

		if save_dir:
			misc.imsave(save_dir+str(i)+"rgb.png",rgb)
			misc.imsave(save_dir+str(i)+"depth.png",depth)
			misc.imsave(save_dir+str(i)+"gt_copy.png",gt_copy)
			misc.imsave(save_dir+str(i)+"predicted_depth.png",gtdepth)
...
if __name__ == '__main__':
	...
	viz_predicted_depth1(sleep =.2,filter_files = " 2017-06-16-19",path = "/media/drc/DATA/chris_labelfusion/RGBDCNN/",model_path = "../models/net_depth_seg_v1.hdf5",save_dir = save_dir)