...
__global__ void Update_Neighborhood_Connectivity_device(
    const int* in_idx_img, int num_labels, int* connectivity_matrix,
    Vector2i img_size);

__global__ void Update_Neighborhood_Connectivity_Lists_device(
    int num_labels, const int* connectivity_matrix, spixel_info* spixel_list,
    Vector2i map_size);
...
seg_engine_GPU::seg_engine_GPU(const settings& in_settings)
  ...
  connectivity_img = new IntImage(Vector2i(spixel_per_row * spixel_per_col,
                                           spixel_per_row * spixel_per_col),
                                  true, true);
...
void gSLICr::engines::seg_engine_GPU::Cvt_Img_Space(UChar4Image* inimg,
  ...
  Cvt_Img_Space_device<<<gridSize, blockSize>>>(inimg_ptr, outimg_ptr, img_size,
                                                color_space);
}
...
void gSLICr::engines::seg_engine_GPU::Update_Neighborhood_Connectivity() {
  int* idx_ptr = idx_img->GetData(MEMORYDEVICE_CUDA);
  int* connectivity_ptr = connectivity_img->GetData(MEMORYDEVICE_CUDA);
  spixel_info* spixel_list_ptr = spixel_map->GetData(MEMORYDEVICE_CUDA);
  ...
  // Zero out connectivity, as we're about to regenerate it.
  cudaMemset(connectivity_ptr, 0, (map_size.x * map_size.y) *
                                      (map_size.x * map_size.y) * sizeof(int));
  Update_Neighborhood_Connectivity_device<<<gridSize, blockSize>>>(
      idx_ptr, map_size.x * map_size.y, connectivity_ptr, img_size);
  Update_Neighborhood_Connectivity_Lists_device<<<gridSize2, blockSize>>>(
      map_size.x * map_size.y, connectivity_ptr, spixel_list_ptr, map_size);
}
...
__global__ void Update_Neighborhood_Connectivity_device(
  ...
  update_neighbor_connectivity(in_idx_img, num_labels, connectivity_matrix,
                               img_size, x, y);
}
...
__global__ void Update_Neighborhood_Connectivity_Lists_device(
  ...
  update_neighbor_connectivity_lists(num_labels, connectivity_matrix,
                                     spixel_list, map_size, x, y);
}
