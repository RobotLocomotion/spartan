...
_CPU_AND_GPU_CODE_ inline void rgb2depth(const gSLICr::Vector4u& pix_in, gSLICr::Vector4f& pix_out)
{
	float depth_mm = (float)(((short unsigned int)pix_in.x) + ((short unsigned int) pix_in.y) << 8);

	pix_out.x = depth_mm;
	pix_out.y = 0.0;
	pix_out.z = pix_in.z; // If nonzero, we will have to ignore this is distance calculations
}
...
_CPU_AND_GPU_CODE_ inline void cvt_img_space_shared(const gSLICr::Vector4u* inimg, gSLICr::Vector4f* outimg, const gSLICr::Vector2i& img_size, int x, int y, const gSLICr::COLOR_SPACE& color_space)
{
	...
	case gSLICr::CIELAB:
		...
		break;
	case gSLICr::DEPTH:
		rgb2depth(inimg[idx], outimg[idx]);
		break;
	}
}
...
// At a given position x, y in the index image (which labels the index
// of the superpixel that the x,y pixel has been corresponded to),
// and given a num_labels^2 connectivity matrix, look at the pixels
// at x+1, y and y+1, x, get their label indices, and assign the
// label_here, label_neighboring entries of the connectivity matrix
// to 1.
_CPU_AND_GPU_CODE_ inline void update_neighbor_connectivity(const int* in_idx_img, int num_labels, int * connectivity_matrix, gSLICr::Vector2i img_size, int x, int y)
{
	int clable_here = in_idx_img[y*img_size.x + x];

	if (x < img_size.x - 1){
		int clable_next = in_idx_img[y*img_size.x + x + 1];
		connectivity_matrix[clable_here*num_labels + clable_next] = 1;
		connectivity_matrix[clable_next*num_labels + clable_here] = 1;
	}
	if (y < img_size.y - 1){
		int clable_next = in_idx_img[(y+1)*img_size.x + x];
		connectivity_matrix[clable_here*num_labels + clable_next] = 1;
		connectivity_matrix[clable_next*num_labels + clable_here] = 1;
	}
}

// At a given position x, y corresponding to a given superpixel index
// (superpixels are reasoned about in a 2-dimensional grid downsampled
// roughly from the original image), scans through the upper half of the
// connecitivity matrix looking for spixels that this spixel is neighbor
// to, and when it finds one, adds it to the list of neighbors for this
// spixel.
_CPU_AND_GPU_CODE_ inline void update_neighbor_connectivity_lists(int num_labels, const int * connectivity_matrix, gSLICr::objects::spixel_info* spixel_list, gSLICr::Vector2i map_size, int x, int y)
{
	int spixel_idx = y * map_size.x + x;

	int num_neighbors = 0;
	for (int i = spixel_idx; i < num_labels; i++){
		if (connectivity_matrix[i*num_labels + spixel_idx] > 0 && num_neighbors < SPIXEL_INFO_MAX_NUM_NEIGHBORS){
			spixel_list[spixel_idx].neighbors[num_neighbors] = i;
			num_neighbors++;
		}
	}

	while (num_neighbors < SPIXEL_INFO_MAX_NUM_NEIGHBORS){
		spixel_list[spixel_idx].neighbors[num_neighbors] = -1;
		num_neighbors++;
	}
}