...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help forcespro_qp_1_contact_point_mex' for details.");
    }    
	if (nlhs > 3) 
	{
	...
	{
    mexErrMsgTxt("PARAMS.QuadraticCost must be of size [4 x 4]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.QuadraticCost, 16);
	...
	{
    mexErrMsgTxt("PARAMS.LinearCost must be of size [4 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.LinearCost, 4);
	...
	copyCArrayToM( output.alpha, mxGetPr(outvar), 4);
