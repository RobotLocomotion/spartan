...
a Python wrapper for a fast solver generated by FORCES Pro v1.6.115
   ...
   OUTPUT = forcespro_qp_3_contact_point_py.forcespro_qp_3_contact_point_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['QuadraticCost'] - matrix of size [12 x 12]
       PARAMS['LinearCost'] - column vector of length 12

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['alpha'] - column vector of size 12

   [OUTPUT, EXITFLAG] = forcespro_qp_3_contact_point_py.forcespro_qp_3_contact_point_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = forcespro_qp_3_contact_point_py.forcespro_qp_3_contact_point_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'forcespro_qp_3_contact_point/lib/forcespro_qp_3_contact_point.so')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'forcespro_qp_3_contact_point/lib/forcespro_qp_3_contact_point.so'))
	csolver = getattr(_lib,'forcespro_qp_3_contact_point_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'forcespro_qp_3_contact_point/lib/libforcespro_qp_3_contact_point.so'))
	csolver = getattr(_lib,'forcespro_qp_3_contact_point_solve')

class forcespro_qp_3_contact_point_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('QuadraticCost', ctypes.c_double * 144),
('LinearCost', ctypes.c_double * 12),
]

forcespro_qp_3_contact_point_params = {'QuadraticCost' : np.array([]),
'LinearCost' : np.array([]),
}
...
class forcespro_qp_3_contact_point_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('alpha', ctypes.c_double * 12),
]

forcespro_qp_3_contact_point_outputs = {'alpha' : np.array([]),
}


class forcespro_qp_3_contact_point_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(forcespro_qp_3_contact_point_params_ctypes), ctypes.POINTER(forcespro_qp_3_contact_point_outputs_ctypes), ctypes.POINTER(forcespro_qp_3_contact_point_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def forcespro_qp_3_contact_point_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES Pro v1.6.115
 ...
 See also COPYING
	...
	# convert parameters
	params_py = forcespro_qp_3_contact_point_params_ctypes()
	...
	outputs_py = forcespro_qp_3_contact_point_outputs_ctypes()
	info_py = forcespro_qp_3_contact_point_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.forcespro_qp_3_contact_point_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.forcespro_qp_3_contact_point_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None )
			else:
				exitflag = _lib.forcespro_qp_3_contact_point_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
			libc.fclose(fp)
	...
	# convert outputs
	for out in forcespro_qp_3_contact_point_outputs:
		forcespro_qp_3_contact_point_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return forcespro_qp_3_contact_point_outputs,int(exitflag),info_py

solve = forcespro_qp_3_contact_point_solve
