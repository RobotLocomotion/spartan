...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help forcespro_qp_3_contact_point_mex' for details.");
    }    
	if (nlhs > 3) 
	{
    ...
    if( mxGetM(par) != 12 || mxGetN(par) != 12 ) 
	{
    mexErrMsgTxt("PARAMS.QuadraticCost must be of size [12 x 12]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.QuadraticCost, 144);
	...
	{
    mexErrMsgTxt("PARAMS.LinearCost must be of size [12 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.LinearCost, 12);
	...
	copyCArrayToM( output.alpha, mxGetPr(outvar), 12);
