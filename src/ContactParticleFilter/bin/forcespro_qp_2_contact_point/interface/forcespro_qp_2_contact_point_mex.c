...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1) 
	{
        mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help forcespro_qp_2_contact_point_mex' for details.");
    }    
	if (nlhs > 3) 
	{
	...
	{
    mexErrMsgTxt("PARAMS.QuadraticCost must be of size [8 x 8]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.QuadraticCost, 64);
	...
	{
    mexErrMsgTxt("PARAMS.LinearCost must be of size [8 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.LinearCost, 8);
	...
	copyCArrayToM( output.alpha, mxGetPr(outvar), 8);
