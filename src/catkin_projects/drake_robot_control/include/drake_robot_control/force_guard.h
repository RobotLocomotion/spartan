...
/// Contains a single method, EvaluateGuard, which checks to see if the guard
/// has been triggered,
/// and additionally reports the fraction of how much it has been triggered
class ForceGuard {
public:
  ForceGuard(const ForceGuardType &type)
      : type_(type), has_been_triggered_(false){};
  ...
  EvaluateGuard(const KinematicsCache<double> &cache_,
                const Eigen::Ref<const Eigen::VectorXd> &q,
                const Eigen::Ref<const Eigen::VectorXd> &tau_external) = 0;

  inline bool HasBeenTriggered() { return has_been_triggered_; }

  inline ForceGuardType get_type() { return type_; }
...
public:
  ...
  EvaluateGuard(const KinematicsCache<double> &cache_,
                const Eigen::Ref<const Eigen::VectorXd> &q,
                const Eigen::Ref<const Eigen::VectorXd> &tau_external) override;
...
/// Checks whether joint_torque_external.norm() >
/// threshold_torque_external.norm()
/// where threshold_torque_external is the external torque gotten by applying
/// the specific
/// force to the specified body location
class ExternalForceGuard : public ForceGuard {
public:
  ExternalForceGuard(const RigidBodyTreed &tree, int idx_body, int idx_world,
                     int idx_expressed_in,
...
public:
  ...
  inline void AddGuard(std::shared_ptr<ForceGuard> guard) {
  ...
  inline std::shared_ptr<ForceGuard> GetTriggeredGuard() {
    return triggered_guard_;
  }

  std::pair<bool, std::pair<double, std::shared_ptr<ForceGuard>>>
  EvaluateGuards(const KinematicsCache<double> &cache_,
                 const Eigen::Ref<const Eigen::VectorXd> &q,
