...
// Current time t
        void TaskSpaceStreamingPlan::Step(
          ...
          PlanStatus prev_plan_status = this->get_plan_status();
          ...
          J_ee_E_ = tree_->geometricJacobian(cache_, world_frame_->get_frame_index(), ee_frame_->get_frame_index(),
                                             ee_frame_->get_frame_index());
          ...
          if (this->use_ee_velocity_mode_) { // velocity goal mode
            ...
            auto R_angular = tree_->relativeTransform(
                    cache_, this->ee_frame_->get_frame_index(),
                    this->angular_velocity_cmd_expressed_in_frame_->get_frame_index()).linear();

            auto R_linear = tree_->relativeTransform(
                    cache_, this->ee_frame_->get_frame_index(),
                    this->linear_velocity_cmd_expressed_in_frame_->get_frame_index()).linear();

            T_WE_E_cmd.head(3) = R_angular * this->angular_velocity_cmd_;
            T_WE_E_cmd.tail(3) = R_linear * this->linear_velocity_cmd_;

          } else { // position goal mode or // linear velocity setpoint mode

            // update setpoint position if we are in that mode
            // essentially just adjust xyz_ee_goal_ using euler integration step
            if (this->setpoint_velocity_mode_ && (prev_plan_status == PlanStatus::RUNNING)){
              xyz_ee_goal_ += setpoint_linear_velocity_cmd_ * dt;
            }
        ...
        void TaskSpaceStreamingPlan::HandleSetpoint(
          ...
          this->have_goal_ = false; // (manuelli): not sure why this is here?
          ...
          this->setpoint_velocity_mode_ = msg->setpoint_linear_velocity_mode;
          ...
          if (this->use_ee_velocity_mode_) {
          ...
          } else if (this->setpoint_velocity_mode_) {



            // set xyz_ee_goal_, xyz_d_ee_goal_, quat_ee_goal_ appropriately
            if (!this->have_processed_first_setpoint_message_)
            {

              tree_->doKinematics(cache_measured_state_);

              // compute current position of end-effector
              auto T_W_E = tree_->relativeTransform(cache_measured_state_, world_frame_->get_frame_index(),
                                                    ee_frame_->get_frame_index());
              xyz_ee_goal_ = T_W_E.translation();

              const auto &R = T_W_E.linear(); // get quaternion from this
              quat_ee_goal_ = Eigen::Quaterniond(T_W_E.linear());

            }


            xyz_d_ee_goal_.setZero();

            // set the velocity
            setpoint_linear_velocity_cmd_ = Eigen::Vector3d(msg->setpoint_linear_velocity.x,
                                                            msg->setpoint_linear_velocity.y,
                                                            msg->setpoint_linear_velocity.z);
            ...
            auto T_W_cmd = tree_->relativeTransform(cache_measured_state_, world_frame_->get_frame_index(),
                                                    position_cmd_expressed_in_frame_->get_frame_index());


            const auto &R = T_W_cmd.linear();
            xyz_ee_goal_ = tree_->transformPoints(
                    cache_measured_state_, xyz_ee_goal_, position_cmd_expressed_in_frame_->get_frame_index(),
                    world_frame_->get_frame_index());
            ...
            if (debug) {
              ...
              std::cout << "expressed in frame index " << position_cmd_expressed_in_frame_->get_frame_index()
                        << std::endl;
          ...
          have_goal_ = true;
          this->have_processed_first_setpoint_message_ = true;
