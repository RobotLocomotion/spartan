#include "schunk_wsg_ros_controller.h"
...
// Grab references to these constants and utilities defined
// for Drake's LCM-specific driver, but don't include the
// entire namespace to avoid collisions.
using drake::manipulation::schunk_wsg::kSchunkWsgNumPositions;
using drake::manipulation::schunk_wsg::kSchunkWsgNumVelocities;
using drake::manipulation::schunk_wsg::kSchunkWsgPositionIndex;
using drake::manipulation::schunk_wsg::kSchunkWsgNumActuators;
using drake::manipulation::schunk_wsg::GetSchunkWsgFeedbackSelector;

SchunkWsgRosController::SchunkWsgRosController() {
  systems::DiagramBuilder<double> builder;

  auto wsg_trajectory_generator =
      builder.AddSystem<SchunkWsgRosTrajectoryGenerator>(
  ...
  auto wsg_controller =
      builder.AddSystem<systems::controllers::PidController<double>>(
          GetSchunkWsgFeedbackSelector<double>(), wsg_kp, wsg_ki, wsg_kd);
  ...
  set_name("SchunkWsgRosController");
}
}
}
}