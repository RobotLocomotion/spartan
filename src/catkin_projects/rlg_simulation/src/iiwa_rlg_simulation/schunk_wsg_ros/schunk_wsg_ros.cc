...
using drake::manipulation::schunk_wsg::SchunkWsgTrajectoryGeneratorStateVector;

SchunkWsgRosTrajectoryGenerator::SchunkWsgRosTrajectoryGenerator(
    int input_size, int position_index)
    : position_index_(position_index),
      target_output_port_(
          this->DeclareVectorOutputPort(
                  BasicVector<double>(2),
                  &SchunkWsgRosTrajectoryGenerator::OutputTarget)
              .get_index()),
      max_force_output_port_(
          this->DeclareVectorOutputPort(
                  BasicVector<double>(1),
                  &SchunkWsgRosTrajectoryGenerator::OutputForce)
              .get_index()) {
  this->set_name("SchunkWsgRosTrajectoryGenerator");
...
void SchunkWsgRosTrajectoryGenerator::OutputTarget(
...
void SchunkWsgRosTrajectoryGenerator::OutputForce(
    const Context<double>& context, BasicVector<double>* output) const {
  const SchunkWsgTrajectoryGeneratorStateVector<double>* traj_state =
      dynamic_cast<const SchunkWsgTrajectoryGeneratorStateVector<double>*>(
          &context.get_discrete_state(0));
...
void SchunkWsgRosTrajectoryGenerator::DoCalcDiscreteVariableUpdates(
    const Context<double>& context,
    const std::vector<const systems::DiscreteUpdateEvent<double>*>&,
    DiscreteValues<double>* discrete_state) const {
  const systems::AbstractValue* input = this->EvalAbstractInput(context, 0);
  DRAKE_ASSERT(input != nullptr);
  const auto& command = input->GetValue<wsg50_msgs::WSG_50_command>();
  ...
  double target_position = -(command.position_mm / 1e3) / 2.;
...
SchunkWsgRosTrajectoryGenerator::AllocateDiscreteState() const {
  return std::make_unique<DiscreteValues<double>>(
      std::make_unique<SchunkWsgTrajectoryGeneratorStateVector<double>>());
}

void SchunkWsgRosTrajectoryGenerator::UpdateTrajectory(
...
SchunkWsgRosStatusSender::SchunkWsgRosStatusSender(int input_size,
                                                   int position_index,
                                                   int velocity_index)
    : position_index_(position_index),
      velocity_index_(velocity_index) {
  this->set_name("SchunkWsgRosStatusSender");
  this->DeclareInputPort(systems::kVectorValued, input_size);
  this->DeclareAbstractOutputPort(&SchunkWsgRosStatusSender::OutputStatus);
}

void SchunkWsgRosStatusSender::OutputStatus(
    const Context<double>& context, wsg50_msgs::WSG_50_state* output) const {
  wsg50_msgs::WSG_50_state& status = *output;
  ...
  status.position_mm = -2 * state->GetAtIndex(position_index_) * 1e3;
  // TODO(sam.creasey) Figure out how to get the actual force from
  // the plant so that we can populate this field.
  status.force = 0;
  status.speed_mm_per_s = -2 * state->GetAtIndex(velocity_index_) * 1e3;
}
