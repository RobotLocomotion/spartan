...
/// Receives ROS WSG_50_command msgs for a Schunk WSG (input port 0)
...
class SchunkWsgRosTrajectoryGenerator : public systems::LeafSystem<double> {
 public:
  ...
  SchunkWsgRosTrajectoryGenerator(int input_size, int position_index);
...
/// Sends ROS WSG_50_state messages for a Schunk WSG.  This
/// system has one input port for the current state of the simulated
/// WSG (probably a RigidBodyPlant).
class SchunkWsgRosStatusSender : public systems::LeafSystem<double> {
 public:
  SchunkWsgRosStatusSender(int input_size, int position_index,
                           int velocity_index);

 private:
  void OutputStatus(const systems::Context<double>& context,
                    wsg50_msgs::WSG_50_state* output) const;
