...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    printf(
        "Use: run_super4pcs_pose_estimator <scene cloud, vtp> <model cloud, "
        "vtp> <config file> <optional output_file>\n");
    exit(-1);
  }
  ...
  // 0.1}});
  rm.publishPointCloud(scene_pts, {"super4pcs", "scene_pts_downsampled"},
                       {{0.1, 1.0, 1.0}});
  // rm.publishPointCloud(model_pts_in, {"model_pts"}, {{0.1, 1.0, 1.0}});
  rm.publishPointCloud(model_pts, {"super4pcs", "model_pts_downsampled"},
                       {{0.1, 1.0, 1.0}});
  ...
  */
  ...
  if (s4pcs_config["overlap_estimation"])
    s4pcs_opts.overlap_estimation =
        s4pcs_config["overlap_estimation"].as<double>();
  if (s4pcs_config["max_normal_difference"])
    s4pcs_opts.max_normal_difference =
        s4pcs_config["max_normal_difference"].as<double>();
  if (s4pcs_config["max_translation_distance"])
    s4pcs_opts.max_translation_distance =
        s4pcs_config["max_translation_distance"].as<double>();
  if (s4pcs_config["max_angle"])
    s4pcs_opts.max_angle = s4pcs_config["max_angle"].as<double>();
  if (s4pcs_config["max_color_distance"])
    s4pcs_opts.max_color_distance =
        s4pcs_config["max_color_distance"].as<double>();
  if (s4pcs_config["terminate_threshold"])
    s4pcs_opts.terminate_threshold =
        s4pcs_config["terminate_threshold"].as<double>();
