...
static Eigen::Matrix<Derived, -1, -1> flatten_MxN(
    const Eigen::Matrix<Derived, rows, cols>& x) {
  Eigen::Matrix<Derived, -1, -1> ret(x.rows() * x.cols(), 1);
  for (int i = 0; i < x.rows();
       i++) {  // for each row, paste that row, in order,
               // as elems in the new column vector
    ret.block(i * x.cols(), 0, x.cols(), 1) =
        x.block(i, 0, 1, x.cols()).transpose();
  }
  return ret;
}
...
static void add_McCormick_envelope(MathematicalProgram& prog,
                                   ...
                                   string corename, double xL, double xH,
                                   double yL, double yH, int M_x, int M_y) {
  MatrixXDecisionVariable x_mat(1, 1);
  x_mat(0, 0) = x;
  MatrixXDecisionVariable y_mat(1, 1);
  y_mat(0, 0) = y;
  MatrixXDecisionVariable w_mat(1, 1);
  w_mat(0, 0) = w;
  ...
  prog.AddLinearEqualityConstraint(MatrixXd::Ones(1, M_x * M_y),
                                   MatrixXd::Ones(1, 1), flatten_MxN(z_uv));

  // Create indicator variables xhat and yhat for each subsection
  auto x_hat = prog.NewContinuousVariables(
      M_x, M_y, (corename + string("_xhat")).c_str());
  auto y_hat = prog.NewContinuousVariables(
      M_x, M_y, (corename + string("_yhat")).c_str());
  // They must sum to their respective variable...
  MatrixXd A_sumconstr = MatrixXd::Ones(1, 1 + M_x * M_y);
  A_sumconstr(0, 0) = -1;
  prog.AddLinearEqualityConstraint(A_sumconstr, MatrixXd::Zero(1, 1),
                                   {x_mat, flatten_MxN(x_hat)});
  prog.AddLinearEqualityConstraint(A_sumconstr, MatrixXd::Zero(1, 1),
                                   {y_mat, flatten_MxN(y_hat)});
  // And respect the range of the region they represent -- which may force to
  // zero if the region isn't active
  // Implemented as a bunch of upper and lower bounds
  MatrixXd A_region_bounds_xhat =
      MatrixXd::Zero(M_x * M_y * 2, M_x * M_y + M_x * M_y);
  MatrixXd A_region_bounds_yhat =
      MatrixXd::Zero(M_x * M_y * 2, M_x * M_y + M_x * M_y);
  MatrixXd lb_zero = MatrixXd::Zero(M_x * M_y * 2, 1);
  MatrixXd ub_inf = MatrixXd::Constant(M_x * M_y * 2, 1,
  ...
  for (int u = 0; u < M_x; u++) {
    for (int v = 0; v < M_y; v++) {
      ...
      A_region_bounds_xhat(2 * k + 1, k) =
          -1.0;  // z(u,v) * xH(u,v) - xhat >= 0
      A_region_bounds_xhat(2 * k + 1, M_x * M_y + k) = xH_uv;
      A_region_bounds_yhat(2 * k + 1, k) =
          -1.0;  // z(u,v) * yH(u,v) - yhat >= 0
      A_region_bounds_yhat(2 * k + 1, M_x * M_y + k) = yH_uv;
      k++;
    }
  }
  prog.AddLinearConstraint(A_region_bounds_xhat, lb_zero, ub_inf,
                           {flatten_MxN(x_hat), flatten_MxN(z_uv)});
  prog.AddLinearConstraint(A_region_bounds_yhat, lb_zero, ub_inf,
                           {flatten_MxN(y_hat), flatten_MxN(z_uv)});

  // And finally, constrain w by the four appropriate surfaces
  // Constraints w, xhats, yhats, z_uvs
  MatrixXd A_w_constraints =
      MatrixXd::Zero(4, 1 + M_x * M_y + M_x * M_y + M_x * M_y);
  const int xhat_s = 1;
  ...
  for (int u = 0; u < M_x; u++) {
    for (int v = 0; v < M_y; v++) {
      ...
      double yL_uv = yL + v * kStepSizeY;
      double yH_uv = yL + (v + 1) * kStepSizeY;
      // w >= sum_{uv} xL(u,v) * y_hat(u,v) + x_hat(u,v) * yL(u,v) -
      // xL(u,v)*yL(u,v)*z(u,v)
      ...
      // w >= sum_{uv} xH(u,v) * y_hat(u,v) + x_hat(u,v) * yH(u,v) -
      // xH(u,v)*yH(u,v)*z(u,v)
      ...
      // w <= sum_{uv} xH(u,v) * y_hat(u,v) + x_hat(u,v) * yL(u,v) -
      // xH(u,v)*yL(u,v)*z(u,v)
  ...
  prog.AddLinearConstraint(
      A_w_constraints, lb_zero, ub_inf,
      {w_mat, flatten_MxN(x_hat), flatten_MxN(y_hat), flatten_MxN(z_uv)});
}
