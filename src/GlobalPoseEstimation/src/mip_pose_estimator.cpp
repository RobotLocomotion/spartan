...
// the check manually.
void publishErrorColorCodedPointCloud(
    const Eigen::Matrix3Xd& scene_pts, const Eigen::Matrix3Xd& all_vertices,
    const DrakeShapes::TrianglesVector& all_faces,
    const vector<int>& face_body_map, const vector<Affine3d>& tfs,
  ...
  for (int i = 0; i < scene_pts.cols(); i++) {
    ...
    for (int j = 0; j < all_faces.size(); j++) {
      ...
      for (int k = 0; k < 3; k++) {
        verts.push_back(tfs[face_body_map[j] - 1] *
                        all_vertices.col(all_faces[j](k)));
      }
  ...
  rm.publishPointCloud(scene_pts, {opt_name, "scene_pts_colorized_incumbent"},
                       colors);
}

void doExactCollisionDetectFromPoints(
    const RigidBodyTree<double>& robot,
    const KinematicsCache<double>& robot_kinematics_cache,
    const Eigen::Matrix3Xd& all_vertices,
    const DrakeShapes::TrianglesVector& all_faces,
    const vector<int>& face_body_map, const Matrix3Xd& scene_pts, VectorXd& phi,
    Matrix3Xd& normal, Matrix3Xd& x, Matrix3Xd& body_x, vector<int>& body_idx) {
...
/*
void mipSolCallbackFunction(
    const MathematicalProgram& prog,
    const drake::solvers::GurobiSolver::SolveStatusInfo& solve_info,
    void* usrdata) {
  ((MIPMultipleMeshPoseEstimator*)usrdata)
      ->handleMipSolCallbackFunction(prog, solve_info);
}
void MIPMultipleMeshPoseEstimator::handleMipSolCallbackFunction(
    const MathematicalProgram& prog,
    const drake::solvers::GurobiSolver::SolveStatusInfo& solve_info) {
  ...
  for (int body_i = 1; body_i < robot_.get_num_bodies(); body_i++) {
    ...
    Vector3d Tf =
        prog.GetSolution(transform_by_object_[body_i - 1].T);  //, sol_i);
    Matrix3d Rf =
        prog.GetSolution(transform_by_object_[body_i - 1].R);  //, sol_i);
    ...
    q_robot.block<3, 1>(6 * (body_i - 1) + 3, 0) =
        rotmat2rpy(est_tf.rotation());
  }

  if (getUnixTime() - last_published_sol_ > 0.05 &&
      q_robot.transpose() * q_robot < 100.0) {
    last_published_sol_ = getUnixTime();
    RemoteTreeViewerWrapper rm;
    rm.publishRigidBodyTree(robot_, q_robot, Vector4d(0.2, 0.5, 1.0, 0.5),
                            {"mip", "latest_sol"});
  }
  ...
  if (solve_info.current_objective < best_sol_objective_yet_) {
    ...
    RemoteTreeViewerWrapper rm;
    rm.publishRigidBodyTree(robot_, q_robot, Vector4d(0.0, 0.8, 0.8, 0.5),
                            {"mip", "incumbent_sol"});
    ...
    publishErrorColorCodedPointCloud(scene_pts_, all_vertices_, all_faces_,
                                     face_body_map_, tfs, "mip");
  }
  ...
  solve_history.push_back(MIPMultipleMeshPoseEstimator::SolveHistoryElem(
      {getUnixTime(), solve_info.reported_runtime, solve_info.best_objective,
       solve_info.best_bound, solve_info.explored_node_count,
       solve_info.feasible_solutions_count}));
  return;
}

void mipNodeCallbackFunction(
    const MathematicalProgram& prog,
    const drake::solvers::GurobiSolver::SolveStatusInfo& solve_info,
    void* usrdata, VectorXd& vals, VectorXDecisionVariable& vars) {
  ((MIPMultipleMeshPoseEstimator*)usrdata)
      ->handleMipNodeCallbackFunction(prog, solve_info, vals, vars);
}
void MIPMultipleMeshPoseEstimator::handleMipNodeCallbackFunction(
    const MathematicalProgram& prog,
    const drake::solvers::GurobiSolver::SolveStatusInfo& solve_info,
  ...
  if (is_finite(q_robot)) {
    if (getUnixTime() - last_published_node_ > 0.1 &&
        (q_robot.transpose() * q_robot) < 1000.) {
      last_published_node_ = getUnixTime();
      RemoteTreeViewerWrapper rm;
      rm.publishRigidBodyTree(robot_, q_robot, Vector4d(0.2, 0.2, 1.0, 0.3),
                              {"mip", "latest_node"});
    }
    ...
    if (optUseICPHeuristic_) {
      icp_search_seeds_lock_.lock();
      if (icp_search_seeds_.size() < 1) icp_search_seeds_.push(q_robot);
      icp_search_seeds_lock_.unlock();
    }
  }
  ...
  if (solve_history.size() == 0 ||
      getUnixTime() - solve_history[solve_history.size() - 1].wall_time > 0.1) {
    solve_history.push_back(MIPMultipleMeshPoseEstimator::SolveHistoryElem(
        {getUnixTime(), solve_info.reported_runtime, solve_info.best_objective,
         solve_info.best_bound, solve_info.explored_node_count,
         solve_info.feasible_solutions_count}));
...
void MIPMultipleMeshPoseEstimator::doICPProcessing() {
  ...
  for (int icp_iter = 0; icp_iter < optICPIters_; icp_iter++) {
    ...
    Matrix3Xd body_x;
    vector<int> body_idx;
    doExactCollisionDetectFromPoints(
        robot_, robot_kinematics_cache, all_vertices_, all_faces_,
        face_body_map_, scene_pts_, phi, normal, x, body_x, body_idx);
    ...
    if (error == 0.0) {
      ...
      if (consecutive_rounds_of_nondecreasing_error > 10) break;
    } else {
    ...
    for (int i = 0; i < robot_.get_num_bodies(); i++) {
      if (num_points_on_body[i] > 0) {
        ...
        Matrix3Xd z(
            3,
            num_points_on_body[i]);  // points, in world frame, near this body
        ...
        for (int j = 0; j < body_idx.size(); j++) {
          ...
          if (body_idx[j] == i) {
            ...
            if (scene_pts_(0, j) == 0.0) {
              cout << "Zero points " << scene_pts_.block<3, 1>(0, j).transpose()
                   << " slipping in at bdyidx " << body_idx[j] << endl;
            }
            avg_dist +=
                (scene_pts_.block<3, 1>(0, j) - x.block<3, 1>(0, j)).norm();
            k++;
          }
        }
        if (k == 0) continue;
        ...
        for (int j = 0; j < body_idx.size(); j++) {
          ...
          if (body_idx[j] == i) {
            ...
            if (optICPRejectionProp_ <= 0 ||
                (scene_pts_.block<3, 1>(0, j) - x.block<3, 1>(0, j)).norm() <=
                    avg_dist * optICPRejectionProp_) {
        ...
        auto J = robot_.transformPointsJacobian(robot_kinematics_cache,
        ...
        for (int j = 0; j < k; j++) {
          ...
          if (POINT_TO_PLANE) {
            ...
            // cout << "Together: " << (z_norms.col(j) *
            // z_norms.col(j).transpose()) << endl;
            f.block(0, 0, nq, 1) -=
    ...
    if (fabs(K) > 0.0) {
      // cut out variables that do not enter at all -- i.e., their row and
      // column of Q, and row of f, are 0
      MatrixXd Q_reduced;
      VectorXd f_reduced;
      ...
      int nq_reduced = 0;
      for (int i = 0; i < nq; i++) {
        if (!(fabs(f[i]) <= 1E-10 && Q.block(i, 0, 1, nq).norm() <= 1E-10 &&
              Q.block(0, i, nq, 1).norm() <= 1E-10)) {
          rows_used[i] = true;
          nq_reduced++;
        }
      }
      // do this reduction (collapse the rows/cols of vars that don't
      // correspond)
    ...
    if (getUnixTime() - last_published_icp > 0.01 &&
        q_robot.transpose() * q_robot < 100.0) {
      rm.publishRigidBodyTree(robot_, q_robot, Vector4d(0.5, 0.2, 0.2, 0.5),
                              {"mip", "icp", "icp_rbt"});
  ...
  for (int i = 0; i < phi.size(); i++) {
    new_objective +=
        fmin(optPhiMax_, (x.col(i) - scene_pts_.col(i)).lpNorm<1>());
  }
...
MIPMultipleMeshPoseEstimator::MIPMultipleMeshPoseEstimator(
    YAML::Node config, YAML::Node modelConfig) {
  ...
  if (config["rotation_constraint_num_faces"])
    optRotationConstraintNumFaces_ =
        config["rotation_constraint_num_faces"].as<int>();
  if (config["rotation_constraint_l1_bound"])
    optRotationConstraintL1Bound_ =
        config["rotation_constraint_l1_bound"].as<double>();

  if (config["allow_outliers"])
    optAllowOutliers_ = config["allow_outliers"].as<bool>();
  if (config["phi_max"]) optPhiMax_ = config["phi_max"].as<double>();
  ...
  if (config["downsample_to_this_many_points"])
    optDownsampleToThisManyPoints_ =
        config["downsample_to_this_many_points"].as<int>();
  ...
  if (config["big_M"]) optBigNumber_ = config["big_M"].as<double>();
  if (config["ICP_prior_weight"])
    optICPPriorWeight_ = config["ICP_prior_weight"].as<double>();
  if (config["ICP_max_iters"]) optICPIters_ = config["ICP_max_iters"].as<int>();
  if (config["ICP_outlier_rejection_proportion"])
    optICPRejectionProp_ =
        config["ICP_outlier_rejection_proportion"].as<double>();
  ...
  if (config["HOD_bins"]) optHODBins_ = config["HOD_bins"].as<int>();
  if (config["HOD_dist"]) optHODDist_ = config["HOD_dist"].as<double>();
  if (config["HOD_weight"]) optHODWeight_ = config["HOD_weight"].as<double>();
  ...
  for (auto iter = modelConfig["models"].begin();
       iter != modelConfig["models"].end(); iter++) {
  ...
  all_faces_.clear();
  face_body_map_.clear();  // Maps from a face index (into all_faces) to an RBT
                           // body index.
  collectBodyMeshesFromRBT(all_vertices_, all_faces_, face_body_map_);
}

void MIPMultipleMeshPoseEstimator::doScenePointPreprocessing(
    const Eigen::Matrix3Xd& scene_pts_in, Eigen::Matrix3Xd& scene_pts_out) {
  ...
  if (optDownsampleToThisManyPoints_ < 0) {
    ...
    VectorXi indices =
        VectorXi::LinSpaced(scene_pts_in.cols(), 0, scene_pts_in.cols());
  ...
  RemoteTreeViewerWrapper rm;
  rm.publishPointCloud(scene_pts_out, {"mip", "scene_pts_downsampled"},
                       {{0.1, 1.0, 0.1}});
}

void MIPMultipleMeshPoseEstimator::doModelPointSampling(Matrix3Xd& pts,
                                                        MatrixXd& B) {
  // Extract vertices and meshes from the RBT.
  Matrix3Xd all_vertices;
  DrakeShapes::TrianglesVector all_faces;
  vector<int> face_body_map;  // Maps from a face index (into all_faces) to an
                              // RBT body index.
  ...
  for (const auto& face : all_faces) {
    ...
    face_cumulative_area.push_back(
        face_cumulative_area[face_cumulative_area.size() - 1] + area);
  }

  // Normalize cumulative areas.
  for (int i = 0; i < face_cumulative_area.size(); i++) {
    face_cumulative_area[i] /=
        face_cumulative_area[face_cumulative_area.size() - 1];
  }
  ...
  while (i < optModelSampleRays_) {
    ...
    // (I'm absolutely sure there's a smarter way to do this with a
    // reflection...)
    double s1 = 1.0;
    double s2 = 1.0;
    while (s1 + s2 >= 1.0) {
...
void MIPMultipleMeshPoseEstimator::collectBodyMeshesFromRBT(
    Eigen::Matrix3Xd& all_vertices, DrakeShapes::TrianglesVector& all_faces,
    ...
    for (const auto& collision_elem : body.get_collision_element_ids()) {
      ...
      if (element->hasGeometry()) {
        ...
        if (geometry.hasFaces()) {
          ...
          all_vertices.conservativeResize(3,
                                          points.cols() + all_vertices.cols());
          all_vertices.block(0, all_vertices.cols() - points.cols(), 3,
                             points.cols()) = points;

          // Append the face descriptions as well, being careful to offset the
          // vertex
...
MIPMultipleMeshPoseEstimator::addTransformationVarsAndConstraints(
    MathematicalProgram& prog, bool world_to_body_direction) {
  KinematicsCache<double> robot_kinematics_cache =
      robot_.doKinematics(q_robot_gt_);
  transform_by_object_.clear();

  for (int body_i = 1; body_i < robot_.get_num_bodies(); body_i++) {
    TransformationVars new_tr;
    char name_postfix[100];
    sprintf(name_postfix, "_%s_%d",
            robot_.get_body(body_i).get_model_name().c_str(), body_i);

    // Spawn translations and bound them to large but finite values.
    new_tr.T =
        prog.NewContinuousVariables<3>(string("T") + string(name_postfix));
    prog.AddBoundingBoxConstraint(-optBigNumber_ * VectorXd::Ones(3),
                                  optBigNumber_ * VectorXd::Ones(3), new_tr.T);

    // Spawn indicator variables for translations that indicate negativity vs
    // positivity
    /*
        auto T_pos_indicators =
       prog.NewBinaryVariables<3>(string("T_pos_")+string(name_postfix));
        auto T_pos =
       prog.NewContinuousVariables<3>(string("T_pos_C_")+string(name_postfix));
        auto T_neg_indicators =
       prog.NewBinaryVariables<3>(string("T_neg_")+string(name_postfix));
        auto T_neg =
       prog.NewContinuousVariables<3>(string("T_neg_C_")+string(name_postfix));
        for (int k=0; k<3; k++) {
          prog.AddLinearEqualityConstraint(T_neg_indicators[k] +
       T_pos_indicators[k], 1.0);
          prog.AddLinearConstraint(T_pos[k] <= T_pos_indicators[k] *
       optBigNumber_);
          prog.AddLinearConstraint(T_neg[k] <= T_neg_indicators[k] *
       optBigNumber_);
    ...
    */
    ...
    auto R_abs = prog.NewContinuousVariables<3, 3>(
        "R_abs");  // these will get pruned if they aren't used// constrain
                   // rotations to ground truth
    auto R_diff = prog.NewContinuousVariables<3, 3>(
        "R_diff");  // these will get pruned if they aren't used// constrain
                    // rotations to ground truth

    auto ground_truth_tf = robot_.relativeTransform(
        robot_kinematics_cache, robot_.get_body(body_i).get_body_index(), 0);
    cout << "GT TF: " << ground_truth_tf.matrix() << endl;
    if (world_to_body_direction) ground_truth_tf = ground_truth_tf.inverse();
    ...
    if (optRotationConstraint_ > 0) {
        ...
        case 2:
          ...
          for (int k = 0; k < 9; k++) {
            ...
            prog.AddLinearConstraint(
                R_diff(k) >= (new_tr.R(k) - ground_truth_tf.rotation()(k)));
            prog.AddLinearConstraint(
                R_diff(k) >= -(new_tr.R(k) - ground_truth_tf.rotation()(k)));
          }
          for (int k = 0; k < 3; k++) {
            prog.AddLinearConstraint(
                (RowVector3d::Ones() * R_diff.col(k))(0, 0) <=
                optRotationConstraintL1Bound_);
            prog.AddLinearConstraint(
                (RowVector3d::Ones() * R_diff.row(k).transpose())(0, 0) <=
                optRotationConstraintL1Bound_);
            // We can trivially upper-bound the L1 norms at sqrt(3)
            prog.AddLinearConstraint(
                (RowVector3d::Ones() * R_abs.col(k))(0, 0) <= sqrtf(3));
            prog.AddLinearConstraint(
                (RowVector3d::Ones() * R_abs.row(k).transpose())(0, 0) <=
                sqrtf(3));
          }
          break;
        case 3:
          addMcCormickQuaternionConstraint(prog, new_tr.R,
                                           optRotationConstraintNumFaces_,
                                           optRotationConstraintNumFaces_);
          break;
        case 4:
          new_tr.R_indicators =
              AddRotationMatrixMcCormickEnvelopeMilpConstraints(
                  &prog, new_tr.R, optRotationConstraintNumFaces_);
          break;
        case 5:
          AddBoundingBoxConstraintsImpliedByRollPitchYawLimits(
              &prog, new_tr.R,
              kYaw_0_to_PI_2 | kPitch_0_to_PI_2 | kRoll_0_to_PI_2);
          break;
        case 6:
      ...
      // Formulas only work for vectors, so implement this constraint
      // column-wise
      for (int i = 0; i < 3; ++i) {
        prog.AddLinearEqualityConstraint(
            new_tr.R.col(i) - ground_truth_tf.rotation().col(i),
            Eigen::Vector3d::Zero());
      }
    }
    /*
        ...
        prog.AddLinearConstraint(x(0, 0) >=  (new_tr.R(0, 0) + new_tr.R(1, 1) +
       new_tr.R(2, 2)) );
        prog.AddLinearConstraint(x(0, 0) >= -(new_tr.R(0, 0) + new_tr.R(1, 1) +
       new_tr.R(2, 2)) );
...
void MIPMultipleMeshPoseEstimator::getInitialGuessFromRobotState(
    const VectorXd& q_robot, VectorXd& vals, VectorXDecisionVariable& vars) {
  KinematicsCache<double> robot_kinematics_cache = robot_.doKinematics(q_robot);
  ...
  for (int body_i = 1; body_i < robot_.get_num_bodies(); body_i++) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      vars.block<3, 1>(vals.size() - 9 + k * 3, 0) =
          transform_by_object_[body_i - 1].R.row(k);
    }

    // Using that rotation, set the indicator variables appropriately, if
    // present
    ...
    if (0 && optRotationConstraint_ == 4) {
      ...
      for (int k = 0; k < K; k++) {
        Matrix3d assigns_pos;
        assigns_pos.setZero();
        Matrix3d assigns_neg;
        assigns_neg.setZero();
        for (int x = 0; x < 3; x++) {
          for (int y = 0; y < 3; y++) {
        ...
        if (k == 0) {
          ...
          for (int row = 0; row < 3; row++) {
            ...
            int num_zeros = 0;
            for (int i = 0; i < 3; i++) num_zeros += (assigns(row, i) == 0);
            if (num_zeros == 0) continue;

            // Try all assignments for the zeros of this row.
            for (int assignment = 0; assignment < 1 << num_zeros;
                 assignment++) {
              // Generate the assignment.
              Matrix3d trial_assignment;
              trial_assignment.setZero();
              int j = 0;
              for (int i = 0; i < 3; i++) {
                if (assigns(row, i) == 0) {
              ...
              Matrix3d assigns_topNrows;
              assigns_topNrows.setZero();
              assigns_topNrows.block(0, 0, row + 1, 3) =
                  (assigns + trial_assignment).block(0, 0, row + 1, 3);
              if (Eigen::FullPivLU<Matrix3d>(assigns_topNrows).rank() >=
                  row + 1) {
  ...
  if (config_["detector_type"].as<string>() == "world_to_body_transforms") {
    // for every scene point, project it down onto the models at the supplied TF
    // to get closest object, and use
    // that face assignment / point assignment as our guess if the distance
    // isn't too great
    VectorXd search_phi;
    ...
    robot_.collisionDetectFromPoints(robot_kinematics_cache, scene_pts_,
                                     search_phi, search_norm, search_x,
                                     search_body_x, search_body_idx, false);
    ...
    for (int i = 0; i < scene_pts_.cols(); i++) {
      ...
      if (is_finite(search_phi.block<1, 1>(i, 0))) {
        for (int j = 0; j < all_faces_.size(); j++) {
          ...
          vector<Vector3d> verts;
          for (int k = 0; k < 3; k++) {
            verts.push_back(tfs[search_body_idx[i] - 1] *
                            all_vertices_.col(all_faces_[j](k)));
          }
          Vector3d new_closest_pt =
              closestPointOnTriangle(verts, scene_pts_.col(i));
    ...
    vals.block(vals.size() - f_outlier0.rows(), 0, f_outlier0.rows(), 1) =
        f_outlier0;
    vars.conservativeResize(vars.size() + f_outlier0.cols());
    vars.block(vars.size() - f_outlier0.rows(), 0, f_outlier0.rows(), 1) =
        f_outlier_;

  } else if (config_["detector_type"].as<string>() ==
             "world_to_body_transforms_with_sampled_model_points") {
    // For every scene point, find closest model point, in terms of l1 norm. Use
    // that.
 ...
 ****************************************************************************/
std::vector<MIPMultipleMeshPoseEstimator::Solution>
MIPMultipleMeshPoseEstimator::doObjectPoseEstimationWithWorldToBodyFormulation(
    const Eigen::Matrix3Xd& scene_pts) {
  KinematicsCache<double> robot_kinematics_cache =
      robot_.doKinematics(q_robot_gt_);
  ...
  for (int i = 0; i < 3; i++) {
    char buf[100];
    sprintf(buf, "alpha_%d", i);
    alpha_.push_back(prog.NewContinuousVariables(robot_.get_num_bodies() - 1,
                                                 scene_pts.cols(), buf));
  }

  // Each row is a set of affine coefficinets relating the scene point to a
  // combination
  ...
  if (!optAllowOutliers_) {
    prog.AddLinearEqualityConstraint(RowVectorXd::Ones(scene_pts.cols()), 0.0,
                                     f_outlier_);
  }
    ...
    for (int face_i = 0; face_i < F_.rows(); face_i++) {
      ...
      for (int i = 0; i < k; i++) {
        for (int j = i + 1; j < k; j++) {
          ...
          if (dist > widest_face) widest_face = dist;
        }
      }
    }
    printf("Found widest face %f\n", widest_face);
    optMaxDistToSameFace = widest_face;
    if (optAllowOutliers_) optMaxDistToSameFace += optPhiMax_ * 2.;
  }
    ...
    for (int k_1 = 0; k_1 < scene_pts.cols(); k_1++) {
      for (int k_2 = 0; k_2 < scene_pts.cols(); k_2++) {
        ...
        if (dist >= optMaxDistToSameFace) {
          prog.AddLinearConstraint(f_.row(k_1).transpose() +
                                       f_.row(k_2).transpose() <=
                                   VectorXd::Ones(f_.cols()));
        }
      }
    }

    // And intentionally constraint that the same point (column of C) can only
    // be used in (N-1) / N of the cases.
    for (int j = 0; j < C_.cols(); j++) {
      prog.AddLinearConstraint(C_.col(j).transpose() *
                                   VectorXd::Ones(C_.rows()) <=
                               (C_.rows() - 10) * VectorXd::Ones(1));
    }
  }
  ...
  // prog.AddLinearCost( (1.0 / (double)scene_pts.cols()) *
  // VectorXd::Ones(scene_pts.cols()) * optPhiMax_, f_outlier_);

  // Constrain slacks nonnegative, to help the estimation of lower bound in
  // relaxation
  prog.AddBoundingBoxConstraint(0.0, std::numeric_limits<double>::infinity(),
                                phi_);
  for (int k = 0; k < 3; k++) {
    for (int i = 0; i < robot_.get_num_bodies() - 1; i++) {
      prog.AddBoundingBoxConstraint(0.0,
                                    std::numeric_limits<double>::infinity(),
                                    {alpha_[k].row(i).transpose()});
    }
  }

  /*
    // Helper variable to indicate the number of unique faces that are active
    per object.
    auto faces_active = prog.NewContinuousVariables(f_.cols(), 1,
    "face_active_counter");
    for (int i = 0; i < f_.cols(); i++){
      prog.AddLinearConstraint(faces_active(i) <= (VectorXd::Ones(f_.rows()) *
    f_.col(i)) (0));
    }
    for (int b = 0; b < B_.rows(); b++){
      prog.AddLinearConstraint(B_.row(b) * faces_active >=
    4.*VectorXd::Ones(1));
    }
  */
  ...
  for (size_t k=0; k<f_.cols(); k++){
    prog.AddLinearConstraint(Eigen::MatrixXd::Ones(1, f_.rows()) * f_.col(k) ==
  VectorXd::Ones(1));
  }
  */
  ...
  for (int i = 0; i < C_.rows(); i++) {
    for (int j = 0; j < C_.cols(); j++) {
      prog.AddLinearConstraint(
          C_(i, j) <= (F_.col(j).transpose() * f_.row(i).transpose())(0, 0));
    }
  }
  ...
  for (int i = 0; i < scene_pts.cols(); i++) {
    ...
    prog.AddLinearConstraint(
        phi_(i, 0) ==
        (RowVectorXd::Ones(robot_.get_num_bodies() - 1) * alpha_[0].col(i))(0,
                                                                            0) +
            (RowVectorXd::Ones(robot_.get_num_bodies() - 1) * alpha_[1].col(i))(
                0, 0) +
            (RowVectorXd::Ones(robot_.get_num_bodies() - 1) * alpha_[2].col(i))(
                0, 0) +
            f_outlier_(i) * optPhiMax_);

    if (i == 0)
      printf(
          "WARNING, UNSURE THAT THIS WORKS FOR MULTIPLE BODIES ANY MORE. "
          "CONVEX HULL REFORM HALF DONE...\n");
    for (int body_i = 1; body_i < robot_.get_num_bodies(); body_i++) {
      ...
      Matrix<Expression, 3, 1> l1ErrorPos =
          transform_by_object_[body_i - 1].R * scene_pts.col(i) +
          transform_by_object_[body_i - 1].T -
          all_vertices_ * C_.row(i).transpose();
      Matrix<Expression, 3, 1> selector =
          Vector3d::Ones() *
          (optBigNumber_ *
           (VectorXd::Ones(1) - B_.row(body_i - 1) * f_.row(i).transpose()));
      for (int k = 0; k < 3; k++) {
        prog.AddLinearConstraint(alpha_[k](body_i - 1, i) >=
                                 l1ErrorPos(k) - selector(k));
        prog.AddLinearConstraint(alpha_[k](body_i - 1, i) >=
                                 -l1ErrorPos(k) - selector(k));
      }
    }
  ...
  prog.SetSolverOption(gurobi_solver.solver_id(), "OutputFlag", 1);
  prog.SetSolverOption(gurobi_solver.solver_id(), "LogToConsole", 1);
  prog.SetSolverOption(gurobi_solver.solver_id(), "LogFile", "loggg.gur");
  prog.SetSolverOption(gurobi_solver.solver_id(), "DisplayInterval", 5);

  prog.SetSolverOption(mosek_solver.solver_id(), "MSK_IPAR_LOG", 1);

  if (config_["gurobi_int_options"]) {
    for (auto iter = config_["gurobi_int_options"].begin();
         iter != config_["gurobi_int_options"].end(); iter++) {
      prog.SetSolverOption(gurobi_solver.solver_id(), iter->first.as<string>(),
                           iter->second.as<int>());
    }
  }
  if (config_["gurobi_float_options"]) {
    for (auto iter = config_["gurobi_float_options"].begin();
         iter != config_["gurobi_float_options"].end(); iter++) {
      prog.SetSolverOption(gurobi_solver.solver_id(), iter->first.as<string>(),
                           iter->second.as<float>());
    }
  }

  if (config_["mosek_int_options"]) {
    for (auto iter = config_["mosek_int_options"].begin();
         iter != config_["mosek_int_options"].end(); iter++) {
      prog.SetSolverOption(mosek_solver.solver_id(), iter->first.as<string>(),
                           iter->second.as<int>());
    }
  }
  if (config_["mosek_float_options"]) {
    for (auto iter = config_["mosek_float_options"].begin();
         iter != config_["mosek_float_options"].end(); iter++) {
      prog.SetSolverOption(mosek_solver.solver_id(), iter->first.as<string>(),
                           iter->second.as<float>());
    }
  }
  for (const auto& tf : transform_by_object_) {
    /*
    Not implemented in drake main yet
    for (const auto& vars : std::get<2>(tf.R_indicators))
      mosek_solver.set_branch_priority({vars.col(0), vars.col(1), vars.col(2)},
                                       10);
  ...
  if (optUseInitialGuess_) {
    ...
    // Corruption should be done from q_gt, then recover maximal coordinates
    // from transform queries,
    ...
    KinematicsCache<double> robot_kinematics_cache_corrupt =
        robot_.doKinematics(q_robot_corrupt);
  ...
  //  prog.SetSolverOption(gurobi_solver.solver_id(), "Cutoff", 50.0);
  // isn't doing anything... not invoking this tool right?
  //  prog.SetSolverOption(gurobi_solver.solver_id(), "TuneJobs", 8);
  //  prog.SetSolverOption(gurobi_solver.solver_id(), "TuneResults", 3);
  // prog.SetSolverOption(gurobi_solver.solver_id(), )

  /* TODO(gizatt) Revive this functionality on new API
  ...
  */
  ...
  printf("Code %d, problem %s solved for %lu scene solved in: %f\n", out,
         problem_string.c_str(), scene_pts.cols(), elapsed);
 ...
 ****************************************************************************/
std::vector<MIPMultipleMeshPoseEstimator::Solution>
MIPMultipleMeshPoseEstimator::
    doObjectPoseEstimationWithWorldToBodyFormulationSampledModelPoints(
        const Eigen::Matrix3Xd& scene_pts) {
  KinematicsCache<double> robot_kinematics_cache =
      robot_.doKinematics(q_robot_gt_);
  ...
  // (and normalize by number of pts for MSE calculation)
  prog.AddLinearCost(
      (1.0 / (double)scene_pts.cols()) * VectorXd::Ones(scene_pts.cols()),
      phi_);
  // Constrain slacks nonnegative, to help the estimation of lower bound in
  // relaxation
  prog.AddBoundingBoxConstraint(0.0, std::numeric_limits<double>::infinity(),
  ...
  for (int i = 0; i < scene_pts.cols(); i++) {
    ...
    for (int body_i = 1; body_i < robot_.get_num_bodies(); body_i++) {
      ...
      Matrix<Expression, 3, 1> selector =
          Vector3d::Ones() *
          (optBigNumber_ *
           (VectorXd::Ones(1) - B_.row(body_i - 1) * C_.row(i).transpose()));
      for (int k = 0; k < 3; k++) {
        prog.AddLinearConstraint(alpha_[k](body_i - 1, i) >=
                                 l1ErrorPos(k) - selector(k));
        prog.AddLinearConstraint(alpha_[k](body_i - 1, i) >=
  ...
  printf("ERROR: HOD description calculation WIP, this branch is broken\n");
  exit(-1);
  /*
  ...
  if (feature_weighting > 0.0) {
    ...
    auto scene_feat_data =
        calculateHODDescriptors(scene_pts_, n_bins, max_dist);
    auto model_feat_data =
        calculateHODDescriptors(all_vertices_, n_bins, max_dist);

    // Penalize L1 error of correspondence
    auto phi_feat =
        prog.NewContinuousVariables(scene_pts_.cols(), 1, "phi_feat");
    auto alpha_feat = prog.NewContinuousVariables(
        scene_feat_data.rows(), scene_pts_.cols(), "alpha_feat");

    // Sum of error terms
    prog.AddLinearCost((feature_weighting / (double)scene_pts.cols()) *
                           VectorXd::Ones(scene_pts_.cols()),
                       phi_feat);

    for (int i = 0; i < C_.rows(); i++) {
      prog.AddLinearConstraint(phi_feat(i) ==
                               (RowVectorXd::Ones(scene_feat_data.rows()) *
                                alpha_feat.col(i))(0, 0));

      // L-1 Norm error of features
      Matrix<Expression, -1, -1> l1Error =
          scene_feat_data.col(i) - model_feat_data * C_.row(i).transpose();
  ...
  */
  ...
  auto scene_feat_data = generateNormalsFromMatrix3Xd(scene_pts_, {"mip"},
  false, true);
  auto model_feat_data = generateNormalsFromMatrix3Xd(all_vertices_, {"mip"},
  false, true);

  // Penalize L1 error of correspondence
  double feature_weighting = 1.0;
  auto phi_feat = prog.NewContinuousVariables(scene_pts_.cols(), 1, "phi_feat");
  //auto alpha_feat = prog.NewContinuousVariables(scene_feat_data.rows(),
  scene_pts_.cols(), "alpha_feat");

  // Sum of error terms
  // feature_weight / N_s * sum_i (1. - phi_feat)
  prog.AddLinearCost( (feature_weighting / (double)scene_pts.cols()) *
  (VectorXd::Ones(phi_feat.rows()) - phi_feat).sum());

  for (int i=0; i<C_.rows(); i++){
    // Unsigned dot product error, for normal vectors
    Expression dotProd = (scene_feat_data.col(i).transpose() * (model_feat_data
  * C_.row(i).transpose()))(0, 0);
...
*/
  /*
    ...
    for (int i = 0; i < scene_pts.cols(); i++) {
      ...
      // This generates a linear expression for the scene point transformation
    into model frame:
      Matrix<Expression, 3, 1> transformed_model_pt_expr =
    transform_by_object_[0].R * scene_pts.col(i) + transform_by_object_[0].T;
      Matrix<Expression, 3, 1> selected_model_pt_expr(3, 1);
      // This generates a linear expression selecting a scene point using our
    permutation matrix:
      // TODO(gizatt) This might be able to be made into a one-liner with
    careful use of a diag() call...
      selected_model_pt_expr.block<1, 1>(0, 0) = C_.row(i) *
    all_vertices_.row(0).transpose();
      selected_model_pt_expr.block<1, 1>(1, 0) = C_.row(i) *
    all_vertices_.row(1).transpose();
      selected_model_pt_expr.block<1, 1>(2, 0) = C_.row(i) *
    all_vertices_.row(2).transpose();

      auto full_cost_expr = (transformed_model_pt_expr -
    selected_model_pt_expr).transpose() * (transformed_model_pt_expr -
    selected_model_pt_expr);
 ...
 ****************************************************************************/
std::vector<MIPMultipleMeshPoseEstimator::Solution>
MIPMultipleMeshPoseEstimator::doObjectPoseEstimationWithBodyToWorldFormulation(
    const Eigen::Matrix3Xd& scene_pts) {
  KinematicsCache<double> robot_kinematics_cache =
      robot_.doKinematics(q_robot_gt_);
  ...
  for (int i = 0; i < model_pts.cols(); i++) {
    prog.AddLinearEqualityConstraint(
        VectorXd::Ones(scene_pts.cols()).transpose() * C.row(i).transpose(),
        VectorXd::Ones(1.0));
  }
  ...
  for (int i = 0; i < model_pts.cols(); i++) {
    // TODO(gizatt) Generalize for more than 1 object.
    // This generates a linear expression for the model point transformation
    // into scene frame:
    Matrix<Expression, 3, 1> transformed_model_pt_expr =
        transform_by_object_[0].R * model_pts.col(i) +
        transform_by_object_[0].T;
    Matrix<Expression, 3, 1> selected_scene_pt_expr(3, 1);
    // This generates a linear expression selecting a scene point using our
    // permutation matrix:
    // TODO(gizatt) This might be able to be made into a one-liner with careful
    // use of a diag() call...
    selected_scene_pt_expr.block<1, 1>(0, 0) =
        C.row(i) * scene_pts.row(0).transpose();
    selected_scene_pt_expr.block<1, 1>(1, 0) =
        C.row(i) * scene_pts.row(1).transpose();
    selected_scene_pt_expr.block<1, 1>(2, 0) =
        C.row(i) * scene_pts.row(2).transpose();

    auto full_cost_expr =
        (transformed_model_pt_expr - selected_scene_pt_expr).transpose() *
        (transformed_model_pt_expr - selected_scene_pt_expr);
    prog.AddQuadraticCost(full_cost_expr);
  }
  ...
  for (int body_i=1; body_i<robot_.get_num_bodies(); body_i++){
    ...
    if (body.has_joint() && typeid(body.getJoint()) == typeid(RevoluteJoint)){
      ...
      // I'm going to assume that body_id and parent_id are good and not
  references
      // to world, as nothing should be revolute-jointed to this world... right?

      // To enforce a pin joint between two bodies described in maximal
  coordinates
      ...
      // (the kinematics cache is used here is not important and shouldn't
  affect results)
      Vector3d p_b_1 = robot_.transformPoints(robot_kinematics_cache, p_a_1,
  body_i, parent_id);
      Vector3d p_b_2 = robot_.transformPoints(robot_kinematics_cache, p_a_2,
  body_i, parent_id);

      // Unfortunately, our decision variable-encoded transforms actually go in
  the other direction --
      // they transform from global frame to body frame. So we implement this
  constrain with
      // one layer of indirection:
      // R_a * z_i + T_a = p_a_i   for z_i = unconstrained 3x1 decision variable
  matrix
      ...
      auto e1_a = transform_by_object_[body_i-1].R * z_a +
  transform_by_object_[body_i-1].T;
      auto e1_b = transform_by_object_[body_i-1].R * z_b +
  transform_by_object_[body_i-1].T;
      auto e2_a = transform_by_object_[parent_id-1].R * z_a +
  transform_by_object_[parent_id-1].T;
      auto e2_b = transform_by_object_[parent_id-1].R * z_b +
  transform_by_object_[parent_id-1].T;
...
std::vector<MIPMultipleMeshPoseEstimator::Solution>
MIPMultipleMeshPoseEstimator::doObjectPoseEstimation(
    const Eigen::Matrix3Xd& scene_pts_in) {
  ...
  if (config_["detector_type"] == NULL) {
    runtime_error(
        "MIPMultipleMeshPoseEstimator needs a detector type specified.");
  } else if (config_["detector_type"].as<string>() ==
