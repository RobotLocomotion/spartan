...
class MIPMultipleMeshPoseEstimator {
 public:
  ...
  /*
    void handleMipSolCallbackFunction(const drake::solvers::MathematicalProgram&
    prog, const drake::solvers::GurobiSolver::SolveStatusInfo& solve_info);
    void handleMipNodeCallbackFunction(const
    drake::solvers::MathematicalProgram& prog, const
    drake::solvers::GurobiSolver::SolveStatusInfo& solve_info,
      Eigen::VectorXd& vals, drake::solvers::VectorXDecisionVariable& vars);
*/

  void doScenePointPreprocessing(const Eigen::Matrix3Xd& scene_pts_in,
                                 Eigen::Matrix3Xd& scene_pts_out);
  ...
  std::vector<TransformationVars> addTransformationVarsAndConstraints(
      drake::solvers::MathematicalProgram& prog, bool world_to_body_direction);

  void getInitialGuessFromRobotState(
      const Eigen::VectorXd& q_robot, Eigen::VectorXd& vals,
      drake::solvers::VectorXDecisionVariable& vars);

  std::vector<Solution> doObjectPoseEstimationWithWorldToBodyFormulation(
      const Eigen::Matrix3Xd& scene_pts);
  std::vector<Solution>
  doObjectPoseEstimationWithWorldToBodyFormulationSampledModelPoints(
      const Eigen::Matrix3Xd& scene_pts);
  std::vector<Solution> doObjectPoseEstimationWithBodyToWorldFormulation(
      const Eigen::Matrix3Xd& scene_pts);
  std::vector<Solution> doObjectPoseEstimation(
      const Eigen::Matrix3Xd& scene_pts);

  //  void doICPProcessing();

  RigidBodyTree<double>& get_robot() { return robot_; }
  ...
  std::string getDetailName() const {
    ...
    my_name << "det_s" << optDownsampleToThisManyPoints_ << "_o"
            << optNumOutliers_ << "_r" << optRotationConstraint_ << "_b"
 ...
 private:
  ...
  std::vector<int> face_body_map_;  // Maps from a face index (into all_faces)
                                    // to an RBT body index.
