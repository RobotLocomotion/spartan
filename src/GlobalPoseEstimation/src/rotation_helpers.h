...
static void addMcCormickQuaternionConstraint(MathematicalProgram& prog,
  ...
  prog.AddLinearEqualityConstraint(MatrixXd::Ones(1, 4), MatrixXd::Ones(1, 1),
                                   {B.block<1, 1>(0, 0), B.block<1, 1>(4, 0),
                                    B.block<1, 1>(7, 0), B.block<1, 1>(9, 0)});
  // Finally, constrain each of the bilinear product pairs with their core
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = i; j < 4; j++) {
      ...
      // Approximate bilinear product with McCormick envelope
      // with logarithmic encoding, using M_x and M_y bins
      Eigen::VectorXd phi_x = Eigen::VectorXd::LinSpaced(M_x, -1., 1.);
      Eigen::VectorXd phi_y = Eigen::VectorXd::LinSpaced(M_y, -1., 1.);
      Eigen::Matrix<drake::symbolic::Expression, -1, 1> Bx =
          prog.NewBinaryVariables(M_x, corename + "_x");
      Eigen::Matrix<drake::symbolic::Expression, -1, 1> By =
          prog.NewBinaryVariables(M_y, corename + "_y");
      auto lambda1 = AddBilinearProductMcCormickEnvelopeSos2(
          &prog, x, y, xy, phi_x, phi_y, Bx, By,
