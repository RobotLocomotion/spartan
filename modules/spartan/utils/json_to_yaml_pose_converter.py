...
stored_poses_yaml_destination = os.path.join(spartanUtils.getSpartanSourceDir(), 'src', 'catkin_projects', 'station_config','RLG_iiwa_1','stored_poses_test.yaml')
data_out = dict()
...
for group in data_in:
	...
	for pose in data_in[group]:
		...
		data_out[group][pose_name] = []
		for k in joint_names:
			joint_angle = pose['joints'][k]
			data_out[group][pose_name].append(joint_angle)
